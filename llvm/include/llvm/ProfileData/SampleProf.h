...
enum class SecFuncProfileFlags : uint32_t {
  SecFlagInvalid = 0,
  // Body samples with zero count are moved behind other body samples, and the
  // count value is omitted. The start of zero count entries are marked by
  // "\x80\x00", which cannot be generated by encodeULEB128 without padding.
  SecFlagCompactZeroEntries = (1 << 0),
  // Use a more compressed way to represent line number and discriminator pair.
  SecFlagCompressLineNumber = (1 << 1),
};
...
static inline void verifySecFlag(SecType Type, SecFlagType Flag) {
  ...
  case SecFuncMetadata:
    ...
    break;
  case SecLBRProfile:
    IsFlagLegal = std::is_same<SecFuncProfileFlags, SecFlagType>();
    break;
  default:
  case SecFuncOffsetTable:
